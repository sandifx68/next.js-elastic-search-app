...
test('user can log in and sign out', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Username' }).fill('cora');
  await page
  ...
  await expect(page.locator('nav')).toContainText('cora');
  await expect(page.locator('nav')).toContainText('Logout');

  await page.getByRole('button', { name: 'Logout' }).click();
  await expect(page.locator('nav')).not.toContainText('cora');
